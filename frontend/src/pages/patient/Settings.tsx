...
export const PatientSettings: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Your personal profile settings have been updated successfully.');
      setPassword('');
    } catch (err: any) {
  ...
  return (
    <div className="w-full h-full flex flex-col space-y-6">
      ...
      <div className="bg-white border border-slate-200 p-5 rounded-3xl shadow-sm">
        <h2 className="text-xl font-black text-slate-900">Account Configurations</h2>
        <p className="text-xs text-slate-455 mt-1">Configure your login credentials and contact parameters.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm text-center space-y-4 lg:col-span-1">
          <div className="h-20 w-20 rounded-full bg-brand-500/10 text-brand-600 border border-brand-500/20 font-black text-3xl mx-auto flex items-center justify-center">
            {name.charAt(0)}
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 text-left space-y-2 text-xs">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-emerald-500 flex items-center">
                <ShieldCheck className="h-4 w-4 mr-0.5" /> Verified
              </span>
            </div>
            <div className="flex justify-between">
